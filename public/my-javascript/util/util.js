...
/*
get:根据名字获取对应的值
set:参数：cookie的名称，cookie的值，可选的用于指定cookie何时应被删除的D阿对象，cookie的可选url路径
可选的域，可选的表示是否要添加secure标志的布尔值
unset：要删除的cookie的名称，可选的路径参数，可选的域参数可选的安全参数
注：没有删除已有cookie的方法，所以需要使用相同的路径/域和安全选项再次设置cookie，并将失效时间设置为过去的时间
*/
var CookieUtil = {
	get:function(name){
		var cookieName = encodeURIComponent(name) + "=",
...
/*
  函数方法传递三个参数：模式的匹配项、模式匹配项在字符串中的位置和原始字符串
*/
