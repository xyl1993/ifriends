...
(function ($) {
    ...
    var isIE = function (ver) {
        ...
        tMain1 = 
            '<div class="input-group add-img-icon {class}">\n' +
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
            	...
            	if(data[0]){
            		...
            		if(data.data == undefined && data.code != 0) {//图片上传失败
	            		if(data.code=='8024'){
	            			data.errorMessage = "上传图片个数不能超过9个";
	            			errMsg += data.errorMessage +'\n';
	            		}else if(data.code == 8023){//图片上传失败,图片格式不正确
	            			data.message = "图片格式不正确，只支持jpg,png,gif";
                			//所需展示失败信息，将上传失败的图片删去
                			errMsg += data.message +'\n';
                			//用户头像
	            			if(self.isHeader){
	            		...
	            		//上传单个图片
	            		if(data.code==0){
	    					self.uploadSucFile[i] = data.data.fileUrlWeb;
...
//                                $status.html(msg);
        ...
        renderBrowse: function () {
            ...
            return '<div class="' + css + '"' + status + '> ' + '<span class="glyphicon glyphicon-plus">'
	         			+'</span>' + ' </div>';
        },
